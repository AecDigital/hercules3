...
export class ChecklistDatabase {
  ...
  initialize(selectionId?: string) {
    this.getSearchOptions(this.searchType, selectionId);
  }

  getSearchOptions = (searchType: string, selectionId?: string) => {
    if (this.searchType === 'Ofertas') {
    this.filterBox.getOptions(searchType).subscribe(res => {
      this.tempData = res.body;
      this.dataChange.next(this.tempData);
      });
      } else if (selectionId) {
        this.filterBox.getEmpresa(selectionId).subscribe((res: any) => {
          console.log(res);
          this.tempData = res.body[0].children;
          this.dataChange.next(this.tempData);
      });
  }
}
...
export class H3SearchTreeDialogComponent {
  ...
  constructor(private filterBox: H3FilterBoxService, private _database: ChecklistDatabase, private dialogRef: MatDialogRef<H3SearchTreeDialogComponent>, @Inject(MAT_DIALOG_DATA) data) {
    this.searchType = data.searchType;
    this.filterBox.getOptions(this.searchType).subscribe(res => {
      this.selectOptions = res.body;
    })
  ...
  // test = () => {
  //   this._database.initialize('2');
  // }

  onChangeSelectedOption = (selectedOption) => {
    if (this.searchType === 'Empresas') {
      console.log(selectedOption);
      this._database.initialize(selectedOption.idEmpresa)
    }
